local SafeString = function(Object)
...
--// Asset downloading
local AssetsConfig = {
	RepoOwner = "Awakenchan",
	RepoName = "GcViewerV3",
	RepoBranch = "main",
	AssetsDirName = "GcViwerV2@Assets", 
	LocalPath = "Sigma Spy/assets"  
}

local HttpService = cloneref(game:GetService("HttpService"))
local req = request or http_request
if not req then
	warn("[Assets] Executor does not support HTTP requests, skipping asset download")
	return
end
...
local function fetchDir(repoPath)
	local url = ("https://api.github.com/repos/%s/%s/contents/%s?ref=%s"):format(
		AssetsConfig.RepoOwner,
		AssetsConfig.RepoName,
		repoPath,
		AssetsConfig.RepoBranch
	)
	local res = req({
		Url = url,
		Method = "GET",
	...
	assert(type(body) == "string", "Invalid GitHub response")
	return HttpService:JSONDecode(body)
end

local function downloadFile(entry, localPath)
	...
	if res and (res.StatusCode == 200 or res.status == 200) then
		writefile(localPath, res.Body or res.body)
	else
		warn("[Assets] Failed to download:", entry.path)
	end
end

local function walk(repoPath, localBase)
	ensureFolder(localBase)
	local entries = fetchDir(repoPath)
	for _, entry in entries do
		local localPath = localBase .. "/" .. entry.name
		if entry.type == "dir" then
			walk(entry.path, localPath)
		elseif entry.type == "file" and entry.download_url then
			downloadFile(entry, localPath)
		end
	end
end

local ok, err = pcall(function()
	walk(AssetsConfig.AssetsDirName, AssetsConfig.LocalPath)
end)
if not ok then
	warn("[Assets] Download failed:", err)
end
getgenv().assetpath = AssetsConfig.LocalPath
...
function(e,f)local g= cloneref(game:GetService(f)) return c(g)end}),{StartAndEnd={Enum.
...
,o=c(cloneref(k:GetMouse())),function(l,m)for n,o in next,m do l[n]=o end return l end,
...
__tostring=function(u)return'Signal: '..SafeString(#u.Connections)..' Connections'
...
UDim2.new(0,6,0,1),Name='Marker'..SafeString(z),Parent=y})end end end s.AddMarker=
...
function()local A,B,C=getfenv(),type,SafeString for D,E in next,w do local F=A[D]
...
ViewY,SafeString(#G.Lines),math.ceil(G.FontSize/2)local S=#Q*R+4*R if H then local
...
SafeString(#aa.Lines),math.ceil(aa.FontSize/2)local ad,ae=#ab*ac+4*ac,aa.Frame.
...
setmetatable(ad,{__index=function(ae,af)local ag=cloneref(game:GetService(af))return ac(ag
...
Accent.Gray,BackgroundTransparency=0.4,BackgroundColor3=aa.Accent.Black}local rs = cloneref(game:GetService('RunService')); rs.RenderStepped:Connect(function(ag)local ah,ai,aj=math.
...
function(l,m)i.TimePosition=m end}local rs = cloneref(game:GetService('RunService')); rs.RenderStepped:
...
'This is indented by 30 more pixels'}end,
Viewport=function(h)local i=aa:
InsertPrefab'R15 Rig'local m=h:Viewport{Size=UDim2.new(1,0,0,200),Clone=true,
Model=i}local n=m.Model n:PivotTo(CFrame.new(0,-2.5,-5))
local rs = cloneref(game:GetService('RunService')); 
rs.RenderStepped:Connect(function(p)
local q=CFrame.Angles(0,math.rad(
...
function a.f()
    local folderPath = getgenv().assetpath or "Sigma Spy/assets"
    ...
    if not getcustomasset or not isfolder or not isfolder(folderPath) then
        return fallback()
    end
    ...
    local function scan(dir)
        ...
        for _, full in entries do
            if isfolder and isfolder(full) then
                scan(full)
            else
                local name = full:match("[\\/]([^\\/]+)%.png$") or full:match("^([^\\/]+)%.png$")
                if name then
                    found[name:lower()] = full
                end
            end
        end
    end
    ...
    if #missing > 0 then
        warn(("[Assets] %d asset(s) not found on disk, using fallbacks for: %s")
...
function(ad,ae,af)local ag=aa:GetChildOfClass(ae,'UICorner')if not ag then local ah;ah,ag=pcall(function()return aa:CreateInstance('UICorner',ae)end)if not ah then return end end ag.CornerRadius=af
...
Text=SafeString(af)function ag:SetLabel(aj)ah.Text=aj return self end end},{
...
'https://github.com/depthso/Dear-ReGui/',Debug=false,PrefabsId=91788667330406,
...
SafeString(n)..(m~=#g and h or'')end return i end function aa:GetValueFromAliases(
...
y,y==v or z==v local C=p:Selectable{Text=SafeString(A),Selected=B,ZIndex=6,
...
FindFirstChildOfClass'UIPadding',z.Label B.Text=SafeString(u)Merge(m,{TabButton=y}
...
ImageTransparency=1 y.BackgroundTransparency=0 if not A then local D;D,A=pcall(function()return ad:CreateInstance('UICorner',x)end)if not D then return v,u end end A.CornerRadius=UDim.new(1,0)else
...
Callback y(x,...)end function n:SetValue(z)v.Text=SafeString(z)self.Value=z return
...
=SafeString(x)self:Update()return self end function n:GetValue()return u.Text end
...
'SeparatorText',n)m:Label{Text=SafeString(o),Visible=o~=nil,Parent=p,LayoutOrder=2
...
q in next,o do local r=m:Bullet(n)r:Label{Text=SafeString(q),LayoutOrder=2,Size=
...
'Label'},true)return self end function n:SetValueText(H)A.Text=SafeString(H)end
...
MergeMetatables(q,y),(y.Combo)local C,D,E=p:Label{Text=SafeString(r),Parent=A,Name
...
end function q:SetValueText(I)C.Text=SafeString(I)end function q:ClosePopup()if
...
self.TitleLabel.Text=SafeString(q)return self end function p:SetPosition(q)self.
...
'SliderCFrame','SliderInt3')o('DragCFrame','DragInt3')
return aa
